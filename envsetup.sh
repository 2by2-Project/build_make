...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/2by2/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/_get_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/2by2/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/_get_abs_build_var_cached/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
function lunch()
{
    ...
    local depsOnly=""
    if [[ $(find ./device -type d -name "$CUSTOM_DEVICE" -print -quit) != "" ]]; then
        depsOnly="true"
    fi

    cd $T > /dev/null
    vendor/2by2/build/tools/roomservice.py $product $depsOnly
    cd - > /dev/null
...
. $T/vendor/2by2/build/envsetup.sh
